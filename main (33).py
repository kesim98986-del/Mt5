...
def _resolve_min_range(avg_price: float) -> float:
    """10-pt hard floor for Gold. Formula: mid±5.0 when triggered."""
    if avg_price >= 1000: return 10.0   # XAU/USD, US100 - 10pt spread
...
def generate_chart(
    ...
    # ══════════════════════════════════════════════════════════════════════
    # CANDLE THICKNESS FIX v3: Very aggressive limiting for thick candles
    # ══════════════════════════════════════════════════════════════════════
    ...
    # Step 2: CLEAR DUPLICATES - unique timestamps only
    df = df.drop_duplicates(subset=["time"], keep="last").reset_index(drop=True)
    
    # Step 3: Remove bad/outlier rows
    df = df[(df["open"] > 0) & (df["high"] > 0) &
            (df["low"]  > 0) & (df["close"] > 0)]
    med = df["close"].median()
    df  = df[(df["close"] > med * 0.5) & (df["close"] < med * 2.0)]
    
    # Step 4: Drop consecutive identical OHLC rows (stale ticks)
    mask = ~(
        (df["open"]  == df["open"].shift(1)) &
        (df["high"]  == df["high"].shift(1)) &
        (df["low"]   == df["low"].shift(1))  &
    ...
    # Step 5: LIMIT DATA - ONLY LAST 40 CANDLES for guaranteed thick bars
    SHOW = 40
    df = df.tail(SHOW).reset_index(drop=True)
    
    if len(df) < 10:
        log.info(f"Chart skipped — only {len(df)} unique candles after cleanup")
        return None
    
    num_candles = len(df)
    log.info(f"Chart rendering with {num_candles} candles")

    # ── Layout (12, 8 figure size for better candle visibility) ──
    ...
    av = fig.add_subplot(gs[1])  # Removed sharex to prevent interference
    ar = fig.add_subplot(gs[2])  # Removed sharex to prevent interference
    ...
    # ══════════════════════════════════════════════════════════════════════
    # DYNAMIC CANDLE WIDTH: Guaranteed thick bars (0.85 minimum)
    # ══════════════════════════════════════════════════════════════════════
    # Fixed width of 0.85 ensures thick candles regardless of count
    BODY_WIDTH = 0.85
    
    doji_min   = actual_range * 0.002  # Larger minimum height for visibility
    for i in bull_idx:
        ...
        am.bar(i, ht, bottom=bot, color=BC, width=BODY_WIDTH,
               edgecolor="#00c853", linewidth=0.6, alpha=0.95, zorder=3)
    for i in bear_idx:
        bot = min(df.loc[i, "open"], df.loc[i, "close"])
        ht  = max(abs(df.loc[i, "close"] - df.loc[i, "open"]), doji_min)
        am.bar(i, ht, bottom=bot, color=RC, width=BODY_WIDTH,
               edgecolor="#d50000", linewidth=0.6, alpha=0.95, zorder=3)

    # Set tight axis limits for maximum candle visibility
    am.set_xlim(-0.5, num_candles - 0.5)
    ...
    ar.set_xlim(-0.5, num_candles - 0.5)
    ar.set_ylim(0, 100)
