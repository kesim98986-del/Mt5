...
def generate_chart(
    ...
    if state.last_signal:
        ...
        # Entry line
        hlines_dict['hlines'].append(sig["entry"])
        hlines_dict['colors'].append('#00e5ff')
        hlines_dict['linestyle'].append('-')
        hlines_dict['linewidths'].append(2)
        # SL line
        hlines_dict['hlines'].append(sig["sl"])
        hlines_dict['colors'].append('#ff5252')
        hlines_dict['linestyle'].append('--')
    ...
    # Build title
    tc_ = ('#00c853' if state.trend_bias == "BULLISH"
           else ('#d50000' if state.trend_bias == "BEARISH" else "#90a4ae"))
    tl = {"live": "📡 LIVE", "entry": "🎯 ENTRY", "exit": "🏁 CLOSED"}.get(chart_type, "")
    blk = " 🚫NEWS" if state.block_trading else ""
    mkt = "🟢" if is_market_open() else "🔴"
    title = (f"{tl} {state.pair_display} · {tf} · {mkt} "
    ...
    if state.active_fvg:
        ...
        if _in_range(fvg["low"]) and _in_range(fvg["high"]):
            ax_main.axhspan(fvg["low"], fvg["high"], alpha=0.1, color='#7c4dff', 
                           hatch='///', zorder=1)
