"""
╔══════════════════════════════════════════════════════════════════════╗
║        SMC SNIPER EA v5.8 — COMPLETE PRODUCTION VERSION             ║
║     Senior Quant SMC | Sniper Brain | News Shield | Broker Connect   ║
║   Deduplication | Robust Chart | Supabase | Full Telegram Support    ║
╚══════════════════════════════════════════════════════════════════════╝
"""
...
# ----------------------------------------------------------------------
# Helper functions (market, session)
# ----------------------------------------------------------------------
def is_market_open():
...
# ----------------------------------------------------------------------
# Chart generation – robust y‑axis, simple matplotlib (no mplfinance issues)
# ----------------------------------------------------------------------
...
def compute_signal(tf="M15"):
    ...
    # TradingView optional – skipped for brevity, but you can add it later
    reason = TradeReason()
...
# ----------------------------------------------------------------------
# Deriv WebSocket and trade execution (with duplicate prevention)
# ----------------------------------------------------------------------
...
def _update_or_append_candle(deque_obj, new_candle, nom):
    epoch = new_candle[0]
    if deque_obj and deque_obj[-1][0] == epoch:
        deque_obj[-1] = new_candle
        log.debug(f"Updated candle epoch {epoch}")
    else:
        # If this is a new M15 candle, log the previous completed candle to Supabase
        if nom == 900 and deque_obj:
            prev = deque_obj[-1]
            if supabase:
                try:
                    supabase.table("sniper_candles").insert({
                        "timestamp": datetime.fromtimestamp(prev[0], tz=UTC).isoformat(),
        ...
        log.debug(f"Appended new candle epoch {epoch}")

def _store(nom, rows):
    if nom == 3600:
        for r in rows: _update_or_append_candle(state.h1_candles, r, nom)
    elif nom == 900:
        for r in rows: _update_or_append_candle(state.m15_candles, r, nom)
        if rows: state.current_price = rows[-1][4]
    elif nom == 300:
        for r in rows: _update_or_append_candle(state.m5_candles, r, nom)
        if rows: state.current_price = rows[-1][4]
    elif nom == 60:
        for r in rows: _update_or_append_candle(state.m1_candles, r, nom)
        if rows: state.current_price = rows[-1][4]

async def _fetch(sym, nom):
    ...
    for ag in GRAN_FALLBACKS.get(nom, [nom]):
        try:
            ...
            await send_req({"ticks_history":sym,"end":"latest","count":1,"granularity":ag,"style":"candles","subscribe":1})
            return len(rows)
        except: continue
    return 0
...
async def handle_msg(msg):
    ...
    if mt == "ohlc":
        c = msg["ohlc"]
        epoch = int(c["epoch"])
        open_ = float(c["open"]); high = float(c["high"]); low = float(c["low"]); close = float(c["close"])
        ...
        if gran == 3600: _update_or_append_candle(state.h1_candles, (epoch,open_,high,low,close), 3600)
        elif gran == 900: _update_or_append_candle(state.m15_candles, (epoch,open_,high,low,close), 900); state.current_price = close
        elif gran == 300: _update_or_append_candle(state.m5_candles, (epoch,open_,high,low,close), 300); state.current_price = close
        elif gran == 60: _update_or_append_candle(state.m1_candles, (epoch,open_,high,low,close), 60); state.current_price = close
    elif mt == "candles":
        gran = int(msg.get("echo_req",{}).get("granularity",0))
        rows = [(int(c["epoch"]), float(c["open"]), float(c["high"]), float(c["low"]), float(c["close"])) for c in msg.get("candles",[])]
        if rows:
            rev = {v:k for k,v in state.gran_actual.items()}
            nom = rev.get(gran, gran)
            _store(nom, rows)
    elif mt == "tick":
...
async def trading_loop():
    ...
    FORCE_TEST_TRADE = False   # Set to True to force a test BUY trade (for debugging)
    while state.running:
        try:
...
async def data_freshness_monitor():
    while state.running:
        await asyncio.sleep(60)
        if state.broker_connected and state.last_ohlc_time and time.time()-state.last_ohlc_time > 300:
            log.warning("No OHLC data for 5 min – resubscribing")
            await subscribe_pair(state.pair_key)

# ----------------------------------------------------------------------
# Telegram polling and command handlers (full)
# ----------------------------------------------------------------------
...
# ----------------------------------------------------------------------
# WebSocket engine and health server
# ----------------------------------------------------------------------
...
async def health(req):
    return web.json_response({"status":"running","version":"5.8"})
...
# ----------------------------------------------------------------------
async def main():
    log.info("Starting SMC SNIPER EA v5.8")
    _load_saved_token()
