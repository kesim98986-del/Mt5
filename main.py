...
# ══════════════════════════════════════════════════════════════════════
# FIX #1: generate_chart with FIXED VERTICAL SCALE (minimum 5.0 pips range)
# ══════════════════════════════════════════════════════════════════════
def generate_chart(candles:deque, tf:str="M15",
    ...
    # ══════════════════════════════════════════════════════════════════════
    # FIX #1: FIXED VERTICAL SCALE — Minimum 5.0 pips range to prevent flickering
    # ══════════════════════════════════════════════════════════════════════
    price_min = df["low"].min()
    price_max = df["high"].max()
    price_range = price_max - price_min
    
    # MINIMUM RANGE = 5.0 pips. If actual range is less, center the chart
    # and add ±2.5 pips to each side for stable visualization
    MIN_RANGE = 5.0  # Minimum vertical range in price units (pips)
    
    if price_range < MIN_RANGE:
        # Price range is too small — enforce minimum 5.0 range
        # Calculate center and set limits to center ± 2.5
        center = (price_max + price_min) / 2.0
        chart_min = center - (MIN_RANGE / 2.0)  # center - 2.5
        chart_max = center + (MIN_RANGE / 2.0)  # center + 2.5
        pad = 0  # No additional padding needed since we're using fixed range
    else:
        # Price range is sufficient — use normal 15% padding
    ...
    # Use actual price_range for doji calculation, with fallback for MIN_RANGE
    actual_range = max(price_range, MIN_RANGE)
    ...
    # ══════════════════════════════════════════════════════════════════════
    # FIX #1: Set Y-axis limits using fixed scale calculation
    # ══════════════════════════════════════════════════════════════════════
    am.set_xlim(-1, len(df)+1)
    am.set_ylim(chart_min, chart_max)  # Use pre-calculated fixed limits
    ...
    # ── News event lines ──
    # Use actual_range for positioning calculations
    for ev in state.news_events:
        ...
        for ci, crow in df.iterrows():
            if crow["time"] >= ep:
                ...
                am.text(ci, chart_max - (actual_range * 0.05), "🔴", fontsize=8, ha="center")
                break
