...
def get_session() -> str:
    ...
    Seamless 24-hour cycle with no gaps:
      0-8   ASIAN
      8-13  LONDON
      13-17 OVERLAP
      17-22 NY
...
def generate_chart(candles:deque, tf:str="M15",
    ...
    # FIX: Fixed vertical scale with minimum 2.0 pips padding
    # This prevents chart flickering and shape changes when price range is small
    MIN_PAD = 2.0  # Minimum padding in price units (pips)
    
    if price_range == 0:
        # If min and max are the same, use minimum padding
        pad = MIN_PAD
    else:
        # Apply 15% padding but enforce minimum of 2.0 pips
        pad = max(price_range * 0.15, MIN_PAD)
    ...
    actual_range = price_range if price_range > 0 else MIN_PAD
