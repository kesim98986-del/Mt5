...
def generate_chart(
    ...
    # ══════════════════════════════════════════════════════════════════════
    # CANDLE THICKNESS FIX: Aggressive data limiting and deduplication
    # ══════════════════════════════════════════════════════════════════════
    
    # Step 1: Convert to DataFrame first
    df = pd.DataFrame(list(candles))
    df.columns = ["time", "open", "high", "low", "close"]
    df = df.astype({"open": float, "high": float, "low": float, "close": float})
    
    # Step 2: CLEAR DUPLICATES - ensure unique timestamps BEFORE limiting
    if "time" in df.columns:
    ...
    # Step 4: Drop consecutive identical OHLC rows
    df = _dedupe_candles(df)
    
    # Step 5: LIMIT DATA - only last 50 candles for clear thick bars
    SHOW = 50
    ...
    # ══════════════════════════════════════════════════════════════════════
    # DYNAMIC CANDLE WIDTH: Thick bars that stay visible regardless of count
    # ══════════════════════════════════════════════════════════════════════
    num_candles = len(df)
    # Width 0.8 for 50 candles, scales down slightly for more candles
    BODY_WIDTH = max(0.6, min(0.8, 40.0 / num_candles))
    ...
    # ── Volume (use same BODY_WIDTH for consistency) ──
