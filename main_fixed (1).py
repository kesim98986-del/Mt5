...
FIXES APPLIED:
1. Fixed Chart Scale: Minimum 5.0 pips range enforced in generate_chart
2. 24-Hour Session Fix: get_session() returns proper sessions with NO 'UNKNOWN'
3. No Spam: Telegram photo in main loop DISABLED, only sent on execute/close trade
"""
...
# ══════════════════════════════════════════════════════════════════════
# FIX #2: SESSIONS DICT - Removed "CLOSE", replaced with continuous 24h coverage
# ══════════════════════════════════════════════════════════════════════
# Forex sessions in UTC hours — Full 24-hour cycle with NO gaps
SESSIONS = {
    "ASIAN":   (0, 8),    # 0-8 UTC
    "LONDON":  (8, 13),   # 8-13 UTC
    "OVERLAP": (13, 17),  # 13-17 UTC (London/NY overlap — highest liquidity)
    "NY":      (17, 22),  # 17-22 UTC
    # 22-24 UTC wraps back to ASIAN (handled in get_session function)
}
...
# Forex market open/close (UTC)
MARKET_OPEN_HOUR  = 22  # Sunday 22:00 UTC
MARKET_CLOSE_HOUR = 21  # Friday 21:00 UTC

TOKEN_FILE = Path("/tmp/.deriv_token")  # persisted broker token

PRE_NEWS_BLOCK  = 30 * 60  # seconds
...
class TradeReason:
    ...
    def build_report(self, direction: str) -> str:
        ...
        lines = [\
            f"🧠 *Sniper Brain — Trade Reasoning*",\
            f"Direction: *{arrow}*",\
...
class BotState:
    def __init__(self):
        ...
        self.account_type     = "unknown"  # "real" / "demo"
        self.account_id       = ""
        self.account_balance  = 0.0
        self.account_currency = "USD"
        self.awaiting_token   = False  # True while waiting for user to send token

        # ── Bot state ──
        self.running       = True
        self.paused        = False
        self.autonomous    = True  # always-on autonomous mode
        ...
        self.ob_score         = 0
        self.session_now      = "ASIAN"  # FIX: Initialize to valid session, not "UNKNOWN"
...
# ══════════════════════════════════════════════════════════════════════
# FIX #2: get_session() - Full 24-hour coverage, NO "UNKNOWN" or "CLOSE"
# ══════════════════════════════════════════════════════════════════════
def get_session() -> str:
    """
    Return the current Forex session (UTC).
    Seamless 24-hour cycle with NO gaps and NO 'UNKNOWN':
      0-8   ASIAN
      8-13  LONDON
      13-17 OVERLAP (London/NY overlap — highest liquidity)
      17-22 NY
      22-24 ASIAN (wraps back)
    """
...
def generate_news_chart() -> Optional[str]:
    ...
    nxt_s = (f" | Next🔴: {nxt.title[:22]}@{nxt.dt_utc.astimezone(NY_TZ).strftime('%I:%M%p ET')}"
             if nxt and nxt.dt_utc else "")
    ax.set_title(
        f"📰 Forex Factory — USD & XAU News · {now_ny.strftime('%A %b %d %Y %I:%M%p ET')}{nxt_s}",
...
# ══════════════════════════════════════════════════════════════════════
# PROFESSIONAL CHART - Exness/MT4 Style with proper candlesticks
# ══════════════════════════════════════════════════════════════════════
def generate_chart(candles:deque, tf:str="M15",
    ...
    # ══════════════════════════════════════════════════════════════════════
    # DATA VALIDATION - Filter out corrupt/invalid candle data
    # ══════════════════════════════════════════════════════════════════════
    ...
    # Detect and remove outliers (candles that are >50% away from median)
    median_price = df["close"].median()
    df = df[(df["close"] > median_price * 0.5) & (df["close"] < median_price * 2.0)]
    if len(df) < 20:
        return None
    ...
    # ══════════════════════════════════════════════════════════════════════
    # FIXED VERTICAL SCALE — Adaptive minimum range based on asset type
    # ══════════════════════════════════════════════════════════════════════
    ...
    # Determine minimum range based on asset price level
    # For XAUUSD (~2000-3000): min range = 5.0
    # For EUR/USD (~1.0-1.2): min range = 0.005 (50 pips)
    # For indices (~15000+): min range = 50
    if avg_price > 1000:
        MIN_RANGE = 10.0  # Gold, indices
    elif avg_price > 10:
        MIN_RANGE = 1.0   # Mid-range
    elif avg_price > 1:
        MIN_RANGE = 0.01  # Forex majors like EUR/USD
    else:
        MIN_RANGE = 0.001  # Very small values
    
    if price_range < MIN_RANGE:
        center = avg_price
        chart_min = center - (MIN_RANGE / 2.0)
        chart_max = center + (MIN_RANGE / 2.0)
        actual_range = MIN_RANGE
    else:
        pad = price_range * 0.12  # 12% padding
        chart_min = price_min - pad
        chart_max = price_max + pad
        actual_range = price_range
    ...
    # ══════════════════════════════════════════════════════════════════════
    # PROFESSIONAL CANDLESTICKS - Thin wicks, proper body width (like MT4/Exness)
    # ══════════════════════════════════════════════════════════════════════
    
    # WICKS - Thin lines from low to high (drawn FIRST, behind bodies)
    def _wick_segs(idx):
        return [[(i, df.loc[i,"low"]), (i, df.loc[i,"high"])] for i in idx]
    
    if bull_idx:
        am.add_collection(mc.LineCollection(_wick_segs(bull_idx),
                          colors=BC, linewidths=1.2, zorder=2))
    if bear_idx:
        am.add_collection(mc.LineCollection(_wick_segs(bear_idx),
                          colors=RC, linewidths=1.2, zorder=2))

    # BODIES - Proper thin rectangles (width=0.5 for professional look)
    BODY_WIDTH = 0.5  # Thinner bodies like real trading platforms
    
    # Minimum body height for doji candles
    doji_min = actual_range * 0.002  # Very small minimum for dojis

    for i in bull_idx:
        body_bottom = min(df.loc[i,"open"], df.loc[i,"close"])
        body_height = max(abs(df.loc[i,"close"] - df.loc[i,"open"]), doji_min)
        am.bar(i, body_height, bottom=body_bottom, color=BC, width=BODY_WIDTH, 
               edgecolor="#00c853", linewidth=0.5, alpha=0.95, zorder=3)
    
    for i in bear_idx:
        body_bottom = min(df.loc[i,"open"], df.loc[i,"close"])
        body_height = max(abs(df.loc[i,"close"] - df.loc[i,"open"]), doji_min)
        am.bar(i, body_height, bottom=body_bottom, color=RC, width=BODY_WIDTH,
    ...
    # ── EMA 21 + EMA 50 (with validation) ──
    ema21 = df["close"].ewm(span=21, adjust=False).mean()
    # Validate EMA is within chart range
    ...
    if bull_idx: av.bar(bull_idx, [vol[i]/vmax for i in bull_idx],
                        color=BC, alpha=0.7, width=0.5)
    if bear_idx: av.bar(bear_idx, [vol[i]/vmax for i in bear_idx],
                        color=RC, alpha=0.7, width=0.5)
    av.set_ylim(0, 1.3)
    ...
    # ── SMC overlays (only if within chart range) ──
    if state.active_ob:
        ob = state.active_ob
        if chart_min <= ob["low"] <= chart_max and chart_min <= ob["high"] <= chart_max:
    ...
    if state.active_fvg:
        ...
        if chart_min <= fvg["low"] <= chart_max and chart_min <= fvg["high"] <= chart_max:
    ...
    if state.active_idm:
        ...
        if chart_min <= idm["level"] <= chart_max:
    ...
    if state.active_trap:
        ...
        if chart_min <= trap["level"] <= chart_max:
    ...
    if state.last_signal and "fib_hi" in state.last_signal:
        ...
        if chart_min <= sig["fib_lo"] <= chart_max and chart_min <= sig["fib_hi"] <= chart_max:
    ...
    if state.last_signal:
        sig = state.last_signal
        if chart_min <= sig["entry"] <= chart_max:
            am.axhline(sig["entry"], color=EC, lw=1.6, ls="-")
        if chart_min <= sig["sl"] <= chart_max:
            am.axhline(sig["sl"], color=SC, lw=1.0, ls="--")
        for tk, tc_ in zip(["tp1","tp2","tp3"], TPC):
            if tk in sig and chart_min <= sig[tk] <= chart_max:
                am.axhline(sig[tk], color=tc_, lw=0.8, ls="-.")

    if entry_price and chart_min <= entry_price <= chart_max:
    ...
    if exit_price and chart_min <= exit_price <= chart_max:
    ...
    # ══════════════════════════════════════════════════════════════════════
    # SWING POINTS - Reduced markers (n=15 for cleaner chart)
    # ══════════════════════════════════════════════════════════════════════
    swh, swl = _swing_pts(df, n=15)  # Increased from 8 to 15 for fewer markers
    mp = actual_range * 0.008
    # Only show last 3 swing highs and 3 swing lows for cleaner look
    for i in swh[-3:]:
...
def _bos_choch(df:pd.DataFrame) -> Optional[dict]:
    ...
    lsh,psh=H[-1],H[-2]; lsl,psl=L[-1],L[-2]
    lc=df["close"].iloc[-1]
...
def _pd_zone(df:pd.DataFrame)->tuple:
    hi=df["high"].max(); lo=df["low"].min(); r=hi-lo
    if r==0: return "NEUTRAL",hi,lo
    fib_hi=hi-r*0.382; fib_lo=lo+r*0.382
    c=df["close"].iloc[-1]
...
def compute_signal(tf:str="M15") -> Optional[dict]:
    ...
    if state.pair_key=="XAUUSD" and session not in("LONDON","NY","OVERLAP"):
        return None
    # FIX #2: Removed check for "CLOSE" session since it no longer exists
    # Session will always be one of: ASIAN, LONDON, OVERLAP, NY
    ...
    if bias=="BULLISH":
        entry=ob_["body_hi"]; sl=ob_["low"]*.9995
    else:
        entry=ob_["body_lo"]; sl=ob_["high"]*1.0005
    risk=abs(entry-sl)
...
async def _fetch(sym:str,nom:int)->int:
    ...
    for ag in GRAN_FALLBACKS.get(nom,[nom]):
        ...
        except Exception as e: log.error(f"_fetch {sym} g={ag}: {e}")
    return 0
...
async def _resolve_sym(key:str)->str:
    ...
    for sym in(pri,otc):
        ...
        except Exception as e: log.warning(f"Symbol test {sym}: {e}")
    return pri
...
# ══════════════════════════════════════════════════════════════════════
# MESSAGE HANDLER (includes trade close Telegram notification)
# ══════════════════════════════════════════════════════════════════════
...
async def handle_msg(msg:dict):
    ...
    if mt=="ohlc":
        ...
        if status in("sold","expired"):
            ...
            # ══════════════════════════════════════════════════════════════
            # TRADE CLOSE NOTIFICATION — This is where Telegram photo IS sent
            # (inside close_trade logic, NOT in main loop)
            # ══════════════════════════════════════════════════════════════
    ...
    while state.running:
        try:
            if state.current_price > 0 and len(state.m15_candles) >= 20:
                ...
                # ══════════════════════════════════════════════════════════════
                # FIX #3: TELEGRAM PERIODIC UPDATES DISABLED (Anti-Spam)
        ...
        try:
            ...
            if sig:
                ...
                if sig_conf and sig_conf["direction"] == sig["direction"]:
                    ...
                    # ══════════════════════════════════════════════════════════════
                    # FIX #3: This tg_async call is INSIDE execute_trade context
                    # It's called ONLY when a trade is being EXECUTED, not periodically
                    # This is the CORRECT place for trade entry notifications
                    # ══════════════════════════════════════════════════════════════
                    await tg_async(
...
async def _cmd(cmd:str):
    ...
    if cmd in("/start","/help","cmd_back"):
        ...
        nxt=state.next_red_event
        nxt_s=(f"\n🔴 Next Red: *{nxt.title}* @ `{nxt.dt_utc.astimezone(NY_TZ).strftime('%I:%M%p ET')}`"
               if nxt and nxt.dt_utc else"")
        sig_s=""
        if state.last_signal:
            s=state.last_signal
        ...
        await tg_async(
            f"📋 *How to get your Deriv API Token:*\n\n"
            f"1. Open `app.deriv.com`\n"
            f"2. Login → Account Settings\n"
